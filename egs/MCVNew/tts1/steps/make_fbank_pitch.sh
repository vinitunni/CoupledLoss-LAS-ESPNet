...
if [ -f $data/segments ]; then
  ...
  fbank_feats="ark:extract-segments scp,p:$scp $logdir/segments.JOB ark:- | compute-fbank-feats $vtln_opts --verbose=2 --config=$fbank_config --allow-downsample=true ark:- ark:- |"
  pitch_feats="ark,s,cs:extract-segments scp,p:$scp $logdir/segments.JOB ark:- | compute-kaldi-pitch-feats --verbose=2 --config=$pitch_config --sample-frequency=48000 ark:- ark:- | process-kaldi-pitch-feats $postprocess_config_opt ark:- ark:- |"
  ...
  fbank_feats="ark:compute-fbank-feats $vtln_opts --verbose=2 --config=$fbank_config --allow-downsample=true scp,p:$logdir/wav.JOB.scp ark:- |"
  pitch_feats="ark,s,cs:compute-kaldi-pitch-feats --verbose=2 --config=$pitch_config --sample-frequency=48000 scp,p:$logdir/wav.JOB.scp ark:- | process-kaldi-pitch-feats $postprocess_config_opt ark:- ark:- |"
